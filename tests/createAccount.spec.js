...
test('Empty fields trigger focus on first name', async ({ page }) => {
  ...
  await createAccountPage.createAccountButton.click();
  const isFirstNameFocused= await createAccountPage.firstName.evaluate((el) => document.activeElement === el);
...
test('Maximum Character Limit Validation', async ({ page }) => {
  const createAccountPage = new CreateAccountPage(page);
  await page.goto('/register/');
  await createAccountPage.maxCharacterLimitValidation('Tester123456789123456', 'Bennington1234566612535616233433423423ewkjckwckejwbckekjwbckebekwbcjkejbw');
  await createAccountPage.createAccountButton.click();
...
test('Duplicate email fail register', async ({ page }) => {
  ...
  await createAccountPage.createAccountButton.click();
  expect(await createAccountPage.getErrorMessageText()).toContain("There is a problem, please check");
});
...
test('Error for email missing domain', async ({ page }) => {
  ...
  await createAccountPage.createAccountButton.click();
  const isEmailFocused= await createAccountPage.email.evaluate((el) => document.activeElement === el);
...
test('Invalid email format', async ({ page }) => {
  ...
  expect(await createAccountPage.getEmailErrorText()).toContain("The email address you entered does not seem");
});
...
test('Invalid password format', async ({ page }) => {
  ...
  await createAccountPage.createAccountButton.click();
  await expect(createAccountPage.messageContainer).toBeVisible();
  expect(await createAccountPage.getPasswordErrorText()).toContain("Password Requirements:");
});
...
test('Invalid password - same password value as first name', async ({ page }) => {
  const createAccountPage = new CreateAccountPage(page);
  await page.goto('/register/');
